...
import { publicApi, type PublicInfraction, type PublicPaymentOrder } from "./api/publicApi";
...
export function PublicHome() {
  return <>
    <Box component="section" className="citizen-hero"><Container maxWidth="xl"><Grid container spacing={6} sx={{ alignItems: "center" }}><Grid size={{ xs: 12, md: 7 }}><Typography className="overline">Portal ciudadano oficial</Typography><Typography variant="h1">Consulta tu multa con seguridad.</Typography><Typography className="hero-lead">Usa el número completo de boleta y la placa. Podrás revisar el estado, conocer el saldo exacto y generar una orden para receptoría sin exponer datos personales.</Typography><Stack direction={{ xs: "column", sm: "row" }} spacing={1.5}><Button component={Link} to="/consulta" variant="contained" size="large" startIcon={<SearchRoundedIcon />}>Consultar una multa</Button><Button component={Link} to="/orden-pago" variant="outlined" size="large" startIcon={<PaymentsOutlinedIcon />}>Abrir una orden</Button></Stack><Stack className="trust-row" direction={{ xs: "column", sm: "row" }}>{["Sin crear una cuenta", "Referencias públicas opacas", "Datos personales protegidos"].map((text) => <Typography key={text}><CheckCircleRoundedIcon />{text}</Typography>)}</Stack></Grid><Grid size={{ xs: 12, md: 5 }}><Paper variant="outlined" className="hero-service-card"><Typography className="overline">Antes de comenzar</Typography><Typography variant="h4">Ten tu boleta a mano</Typography><Typography color="text.secondary">La consulta exige la combinación exacta de boleta y placa. Por seguridad no se admiten búsquedas únicamente por placa.</Typography><Button component={Link} to="/consulta" fullWidth variant="contained" endIcon={<ArrowForwardRoundedIcon />}>Iniciar consulta segura</Button><Typography className="privacy-caption"><LockOutlinedIcon /> Nunca mostramos nombres, identificación, agente ni evidencia privada.</Typography></Paper></Grid></Grid></Container></Box>
